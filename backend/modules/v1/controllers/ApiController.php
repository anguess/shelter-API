...
class ApiController extends Controller
{
    ...
    public function behaviors()
    {
     ...
     'except' => ['login', 'signup', 'getrequestedinfo', 'getallshelters','getrequestedinfov2', 'signupshelter', 'shelterconfig', 'shelterupdateconfig', 'updateshelterapprovalview', 'updateshelterapprovalupdate', 'getallsheltersneedactivation']
        ];
...
// Get all shelter by admin ethire approved or not
     ...
     * Author: Annette
     * paramters: shelter_id, new status -- 1 = approved or 2 = suspended, role must equal superuser only
    ...
    /**
     * Action Getrequestedinfov
     * This function send the type of shelter to the homeless user.  if want all shelter types then 5 is sent else will search for Women, Men, Youth or family    
     * Author: Annette
     * paramters: Shelter type searching for
     * return : shelters info for that type
     * @return string
     */
    ...
    public function actionGetrequestedinfov2()
    {
        ...
        if ($requestedShelterType['sheltertype'] == 5) {
            //grab all shelters types from shelter_Detail_table
            ...
            $response1 = (new \yii\db\Query())
                ...
                // var_dump($response1);  //  var_dump #1   

         
            $returnedArray = [];
// Get all shelter_Types for all shelters for sheltertype = 5
          ...
          //request
                    $response2 = (new \yii\db\Query())
                        ->select([
                            ...
                            'shelter_phone', 'shelter_long', 'shelter_lat', 'shelter_county', 'shelter_approved'
                        ])
                        ->from('shelter_table')
...
//           
            ...
            $response1 = (new \yii\db\Query())
                ->select('*')
                ->from('shelter_detail_table')
                ->where(['shelter_type_id' => $requestedShelterType['sheltertype']])
                ->all();
